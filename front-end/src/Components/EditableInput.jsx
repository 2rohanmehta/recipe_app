...
function EditableInput({ value: initialValue, type = "", onSave, ...props }) {
  ...
  const handleSave = (event) => {
    ...
    onSave(value);
    axios
      .post("http://localhost:8080/api/updateDescription", {
        description: value,
        userID: userIDedit,
      })
      .then((result) => {
  ...
  const handleBlur = (event) => {
    if (
      event.relatedTarget &&
      event.relatedTarget.className.includes("save-button")
    ) {
    ...
    <div className="flex justify-center items-center">
      <span className="relative">
        ...
        {isEditMode && (
          <>
            <div
              style={{ color: getCharCountColor(), transition: "color 0.5s" }}
              className="absolute bottom-[-5px] right-0 text-sm"
            >
              {charCount}/2000
            </div>
            <ButtonIcon
      ...
      {showSaveMessage && (
        <div className="text-green-500 absolute bottom-full left-1/2 transform -translate-x-1/2 -mb-2 text-xs">
          Description saved!
        </div>
      )}
    </div>
